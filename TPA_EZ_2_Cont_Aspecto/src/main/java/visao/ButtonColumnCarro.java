...
public class ButtonColumnCarro extends AbstractCellEditor implements
		TableCellRenderer, TableCellEditor, ActionListener
{
	private static final long serialVersionUID = 1L;
	private JTable table;
	private JButton button;
	private DialogTabelaCarro dialogTabelaCarro;
	...
	public ButtonColumnCarro(JTable table, int coluna, 
			            DialogTabelaCarro dialogTabelaCliente, 
			            DialogCarro dialogCarro)
	{
		//super();
		this.table = table;
		this.dialogTabelaCarro = dialogTabelaCliente;
	...
	public void actionPerformed(ActionEvent e)
	{
		try
		{
			Carro umCarro = CarroService.recuperaUmCarroComMotorista((Long)table
				.getValueAt(table.getSelectedRow(), 0));
			dialogCarro.designateCarroAFrame(umCarro);
			dialogCarro.editavel();
			dialogTabelaCarro.dispose();
		} 
		catch (CarroNaoEncontradoException e1)
		{ 
			dialogCarro.novo();
			dialogTabelaCarro.dispose();
			JOptionPane.showMessageDialog(dialogCarro, "Cliente não encontrado", "", JOptionPane.ERROR_MESSAGE);
		}
	}
}
